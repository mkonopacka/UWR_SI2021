...
if __name__ == '__main__':
    with open('zad_input.txt', "r") as file_in:
        input = []
        lines = file_in.read().splitlines()
        nrows, ncols = int(lines[0][0]), int(lines[0][2])
        rows = [list(map(int, x.split())) for x in lines[1:1+nrows]]
        cols = [list(map(int, x.split())) for x in lines[1+nrows:]]
        # solved = solve(nrows, ncols, rows, cols)
        # print('________________________________________________________________________________')
        # print(solved)
        # with open('zad_output.txt', 'w') as file_out:
        #     draw(solved, file = file_out)
